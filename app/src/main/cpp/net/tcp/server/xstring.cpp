...
static void string_extendby(struct xstring *sb, int len) {
    char *buf;
...
static void string_vprintf(struct xstring *sb, const char *fmt, va_list ap) {
    int num_required;
    while ((num_required = vsnprintf(sb->buf + sb->NUL, sb->buflen - sb->NUL, fmt, ap)) >=
           sb->buflen - sb->NUL)
...
void string_scatf(struct xstring *sb, const char *fmt, ...) {
    va_list ap;
    va_start(ap, fmt);
